...
async def _get_jwks_client(
    ...
    if discovered_issuer != oidc_cfg.issuer:
        logger.warning(
            f'OIDC discovery issuer {discovered_issuer!r} does not match {oidc_cfg.issuer!r}',
        )
        raise aiohttp.web.HTTPUnauthorized
    ...
    async def get(self):
        ...
        - in: query
          ...
          schema:
            type: string
          description: >
            GitHub access token (requires api_url) or OIDC bearer token (without api_url).
        - in: query
          name: api_url
          required: false
          schema:
        ...
        - in: header
          ...
          schema:
            type: string
          description: >
            OIDC bearer token in the form "Bearer <token>". Alternative to passing the
            token as access_token without api_url. No refresh token is issued for OIDC logins.
        responses:
          "200":
